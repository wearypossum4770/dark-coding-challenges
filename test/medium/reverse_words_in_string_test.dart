...
import 'package:dark_coding_challenges/medium/reverse_words_in_string.dart';

class ReverseWordsInStringTestCases {
  ReverseWordsInStringTestCases();
}
...
void main() {
  group("Feature:", () {
    final testCases = [
      ...
      ["  spaces   between   words  ", "words between spaces"],
      ["word", "word"],
      ["  word  ", "word"],
      ["a b", "b a"],
      ["a  b", "b a"],
      ...
      ["camelCase pascalCase", "pascalCase camelCase"],
      ["abc123 456def", "456def abc123"],
      ["foo1 bar2 baz3", "baz3 bar2 foo1"],
      ["a1b2c3 d4e5f6 g7h8i9", "g7h8i9 d4e5f6 a1b2c3"],
      ["123 456 789", "789 456 123"],
      ["1", "1"],
      [" 1 ", "1"],
      ["  1  2  3  ", "3 2 1"],
      ["the   sky   is   blue", "blue is sky the"],
      ["Alice Bob Charlie Dave Eve", "Eve Dave Charlie Bob Alice"],
      ["foo bar baz qux quux corge grault garply waldo fred", "fred waldo garply grault corge quux qux baz bar foo"],
      ["  leading", "leading"],
      ["trailing  ", "trailing"],
      ["  both  ", "both"],
      ["a  b  c  d  e", "e d c b a"],
      ["ab cd ef gh ij kl mn op qr st", "st qr op mn kl ij gh ef cd ab"],
    ...
    for (final step in testCases) {
      final s = step[0];
      final expected = step[1];
      test("\n\tGiven: the string ${s.substring(0, 1)}.\n\tThen: after reversing it is ${expected.substring(0, 1)}.", () {
        String actual = reverseWordsInString(s);
