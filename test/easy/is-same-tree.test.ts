...
import { isSameTree } from "@/src/easy/is-same-tree";
...
describe("isSameTree", () => {
	...
	])("\n\tGiven: the trees $p and $p.\n\tThen they are equal %p.", (a: string, b: string, expected: boolean) => {
		const p = TreeNode.fromString(a);
		const q = TreeNode.fromString(b);
