...
class KodeeMaskingPersonallyIdentifiableInformationTest {
    @ParameterizedTest
    @MethodSource("maskingPersonallyIdentifiableInformationData")
    fun kodeeMaskingPersonallyIdentifiableInformationTest(
        s: String,
        expected: String,
    ) {
        val result = kodeeMaskingPersonallyIdentifiableInformation(s)
        assertEquals(expected, result)
    }

    companion object {
        @JvmStatic
        fun maskingPersonallyIdentifiableInformationData(): Stream<Arguments> =
            Stream.of(
