...
class KodeeArrayContainsDuplicateTest {
    @ParameterizedTest
    @MethodSource("arrayContainsDuplicateData")
    fun kodeeArrayContainsDuplicateTest(
        nums: IntArray,
        expected: Boolean,
    ) {
        val result = kodeeArrayContainsDuplicate(nums)
        assertEquals(expected, result)
    }

    companion object {
        @JvmStatic
        fun arrayContainsDuplicateData(): Stream<Arguments> =
            Stream.of(
                ...
                Arguments.of(intArrayOf(1, 3, 4, 2, 2), true),
                Arguments.of(intArrayOf(3, 4, 1, 4, 2), true),
                Arguments.of(intArrayOf(3, 1, 3, 4, 2), true),
                Arguments.of(intArrayOf(2, 1, 2), true),
                Arguments.of(intArrayOf(1, 5, 3, 4, 2, 5), true),
                Arguments.of(intArrayOf(1, 2, 3), false),
                Arguments.of(intArrayOf(3, 4, 1, 4, 1), true),
                Arguments.of(
                    intArrayOf(
                        // spotless:off
