...
func TestArrayContainsDuplicate(t *testing.T) {
	cases := []struct {
		nums     []int
		expected bool
	}{
	...
	for _, c := range cases {
		t.Run("", func(t *testing.T) {
			result := ArrayContainsDuplicate(c.nums)
			if result != c.expected {
				t.Errorf("ArrayContainsDuplicate(%b) == %v, expected %v", c.nums, result, c.expected)
			}
