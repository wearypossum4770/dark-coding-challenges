...
func TestRemoveElement(t *testing.T) {
	...
	for i, c := range cases {
		criteria := fmt.Sprintf("\n\tTest Case: %d\n\tGiven: the array %v.\n\tAnd: the target %d.\n\tThen: it will have %d non-target element.\n\tResulting in %v", i, c.nums, c.val, c.expected, c.result)
		t.Run(criteria, func(t *testing.T) {
			result := RemoveElement(c.nums, c.val)
			if result != c.expected {
				t.Errorf("RemoveElement(%v, %d) == %v, expected %v", c.nums, c.expected, result, c.expected)
			}
